...
import {
    navigateToPressureVehiclesProductsPageOnMobile,
...
import {navigateToPressureVehiclesProductsPageOnTablet} from "../../utils/navigation/navigateProductsMenuOnTablet";

test.describe("pressure vehicles products suite", () => {
    test.describe("desktop version", () => {
        ...
        test.describe("common elements", () => {
            test("display products grid with product titles and images", async () => {
                const productsContainer = desktopPage.getByTestId("water-sewage-product-container-pressure-vehicles").first();
                await productsContainer.waitFor({state: "visible"});
                await expect(productsContainer).toBeVisible({ timeout: 3000 });

                const pressureVehiclesProductsGridContainer = desktopPage.getByTestId("water-sewage-product-grid-container-pressure-vehicles").first();
                await pressureVehiclesProductsGridContainer.waitFor({state: "visible"});
                await expect(pressureVehiclesProductsGridContainer).toBeVisible({ timeout: 3000 });

                const productsLayoutContainer = desktopPage.getByTestId("water-sewage-product-desktop-layout-container-pressure-vehicles").first();
        ...
        test.describe("english version", () => {
            test("display breadcrumbs for pressure vehicles products in english", async () => {
                const breadcrumbsContainer = desktopPage.getByTestId("water-sewage-product-breadcrumbs-container-pressure-vehicles").first();
                ...
                expect(breadcrumbCurrentPageItemText).toBe("Pressure Vehicles");
            });
            ...
            test("display products header with the hero image and title in english", async () => {
                ...
                expect(heroImageTitleText).toBe("Multipurpose vehicles for pressure sewer cleaning");
        ...
        test.describe("polish version", () => {
            ...
            test("display breadcrumbs for pressure vehicles products in polish", async () => {
                const breadcrumbsContainer = desktopPage.getByTestId("water-sewage-product-breadcrumbs-container-pressure-vehicles").first();
                await breadcrumbsContainer.waitFor({state: "visible"});
                await expect(breadcrumbsContainer).toBeVisible({ timeout: 3000 });
                ...
                const breadcrumbCurrentPageItemText = await breadcrumbCurrentPageItem.textContent();
                expect(breadcrumbCurrentPageItemText).toBe("Samochody ciśnieniowe");
            });
            ...
            test("display products header with the hero image and title in polish", async () => {
                ...
                const heroImageTitleText = await heroImageTitle.textContent();
                expect(heroImageTitleText).toBe("Wielofunkcyjne pojazdy do ciśnieniowego czyszczenia kanalizacji");
    ...
    test.describe("mobile version", () => {
        ...
        test.beforeEach(async ({browser}) => {
            ...
            await navigateToPressureVehiclesProductsPageOnMobile(mobilePage);
        });

        test.describe("common elements", () => {
            test("display products grid with product titles and images", async () => {
                const productsGridContainer = mobilePage.getByTestId("water-sewage-product-mobile-layout-container-pressure-vehicles").first();
        ...
        test.describe("polish version", () => {
            ...
            test.beforeEach(async ({browser}) => {
                ...
                await navigateToPressureVehiclesProductsPageOnMobile(mobilePage, Language.PL);
            });
    ...
    test.describe("tablet version", () => {
        ...
        test.describe("common elements", () => {
            test("display products grid with product titles and images", async () => {
                const productsGridContainer = tabletPage.getByTestId("water-sewage-product-desktop-layout-container-pressure-vehicles");
                await productsGridContainer.waitFor({state: "visible"});
                await expect(productsGridContainer).toBeVisible({ timeout: 3000 });
        ...
        test.describe("polish version", () => {
            ...
            test.beforeEach(async ({browser}) => {
                ...
                await navigateToPressureVehiclesProductsPageOnTablet(tabletPage, Language.PL);
            });
